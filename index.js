...
const {log} = require("./NodeBuilder/PlasmaLogger");
...
const printErrorLine = (line) => {
  log.error(`\t- ${line}`, "Console:Err");
  process.exitCode = 1;
};

const printLogLine = (line) => {
  if (line.match(/- not found|- [fF]ailed/gu)) {
    log.warn(`\t+ ${line}`, "Console:Warn");
  } else {
    log.debug(`\t+ ${line}`, "Console:Out");
  }
};
...
const runClangFormat = async (options, sourceFiles) => {
  log.info("Running Clang Format...", "runClangFormat");
...
const preventNoOutputTimeout = () => {
  const interval = setInterval(() => {
    log.debug("Request in Progress...", "preventNoOutputTimeout");
  }, 1000 * 10);
...
const format = async (options) => {
  ...
  log.info(`Formatted | Elapsed Time [${new Date(new Date() - formatStart).toGMTString().split(" ")[4]}]`, "format");
};

const build = async (options) => {
  log.info("Building...", "build");
  if (!ensureCommandExists("cmake")) {
    log.fatal("CMake command does not exist or could not be executed.", "build");
    return;
  }
  const combo = determineCmakeCombo(options);
  const buildDir = activateBuildDir(combo);
  ...
  log.debug(JSON.stringify(cmakeVariables), "build:cmakeVariables");
  log.debug(buildDir, "build:buildDir");
  log.debug(JSON.stringify(combo), "build:combo");
  ...
  endPnot();
  log.info(`Built | Build Time [${new Date(new Date() - buildStart).toGMTString().split(" ")[4]}]`, "build");
};
...
const prebuilt = async (options) => {
  const prebuildStart = new Date();
  log.info("Copying Prebuilt Content...", "Prebuilt");
  const combo = determineCmakeCombo(options);
  ...
  log.info(`Copied Prebuilt Content | Elapsed Time [${new Date(new Date() - prebuildStart).toGMTString().split(" ")[4]}]`, "Prebuilt:Copy");
};
...
const documentation = async () => {
  log.info("Running Doxygen...", "documentation");
  if (!ensureCommandExists("doxygen")) {
    log.error("Doxygen command not found or could not be executed", "documentation");
    return;
  }
